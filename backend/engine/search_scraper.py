"""
search_scraper.py — Instagram Search Engine (keyword / hashtag)

Strategi:
  • Hashtag  → /api/v1/tags/web_info/ (top + recent) + /api/v1/tags/{tag}/sections/ (paginasi recent)
  • Keyword  → /api/v1/web/search/topsearch/ (cari hashtag relevan) → agregasi top posts → sort by likes

Auth: pakai cookie session yang sudah disimpan (cookie_injector), profil Chrome TERPISAH
      (chrome_profile_search) supaya tidak bentrok dengan scraper_post.py.

Semua hit API dilakukan via in-browser fetch (CDP) lebih dulu, fallback ke requests.Session.
"""
import os
import re
import json
...
PROFILE_DIR = os.getenv("SEARCH_PROFILE_DIR", "chrome_profile_search")  # TERPISAH dari scraper_post
...
class InstagramSearchScraper:
    ...
    def initialize_browser(self):
        ...
        if "login" in page.url:
            raise RuntimeError(
                "Redirect ke login — session expired. Inject cookie ulang lewat /api/auth/cookies."
            )
    ...
    def _close_popups(self):
        ...
        for selector in [
            ...
            "text=Batal", "button:has-text('Not Now')",
        ]:
            try:
    ...
    # ── API HELPERS (CDP first, requests fallback) ─────────────
    ...
    def _api_get(self, path: str, retries: int = 2) -> Optional[dict]:
        for attempt in range(retries + 1):
            data, status = self._cdp_get(path)
            if status == 429:
                print(Fore.YELLOW + "   ⚠️  429 (CDP), tunggu 60s..."); time.sleep(60); continue
            if data is not None:
                return data
            url = f"https://www.instagram.com{path}" if path.startswith("/") else path
            rdata, rstatus = self._sess_get(url)
            if rstatus == 429:
                print(Fore.YELLOW + "   ⚠️  429 (REST), tunggu 60s..."); time.sleep(60); continue
    ...
    def _api_post_form(self, path: str, form: dict, retries: int = 1) -> Optional[dict]:
        for attempt in range(retries + 1):
            ...
            if data is not None:
                return data
            rdata, rstatus = self._sess_post(f"https://www.instagram.com{path}", form)
            if rstatus == 429:
                time.sleep(60); continue
            if rdata is not None:
                return rdata
    ...
    def _normalize_hashtag(raw: str) -> str:
        ...
        s = re.sub(r"\s+", "", s)
        s = re.sub(r"[^0-9A-Za-z_\u00C0-\uFFFF]", "", s)  # alnum + underscore + unicode letters
        return s.lower()
    ...
    def _parse_media(self, media: Dict, source: str, rank: int) -> Optional[Dict]:
        ...
        return {
            ...
            "view_count":        int(media.get("view_count") or media.get("play_count") or media.get("ig_play_count") or 0),
    ...
    # ── TOPSEARCH (keyword → hashtags/users/places) ────────────
    def _topsearch(self, query: str) -> Dict:
        out = {"hashtags": [], "users": [], "places": []}
        path = f"/api/v1/web/search/topsearch/?context=blended&query={quote(query)}&include_reel=true"
        ...
        for u in (data.get("users", []) or []):
            ...
            if not un:
                continue
            out["users"].append({
                "username": un, "full_name": us.get("full_name", ""),
        ...
        for p in (data.get("places", []) or []):
            ...
            if not nm:
                continue
            out["places"].append({
                "name": nm, "address": loc.get("address", ""), "city": loc.get("city", ""),
            })
        return out

    # ── HASHTAG PAGINATION ─────────────────────────────────────
    def _fetch_hashtag_sections(self, tag, max_id, next_media_ids, page_num) -> Optional[dict]:
        path = f"/api/v1/tags/{quote(tag)}/sections/"
        form = {
        ...
        return self._api_post_form(path, form)

    def _collect_hashtag_posts(self, tag, max_posts, include_top, include_recent, recent_pages) -> Dict:
        out = {"posts": [], "media_count": 0, "formatted_media_count": "",
               "top_count": 0, "recent_count": 0, "error": None}

        info = self._api_get(f"/api/v1/tags/web_info/?tag_name={quote(tag)}")
        if not info or "data" not in info:
            out["error"] = "web_info_failed"
            return out

        d = info.get("data", {}) or {}
        out["media_count"] = int(d.get("media_count", 0) or 0)
        out["formatted_media_count"] = d.get("formatted_media_count", "") or self._fmt_count(out["media_count"])
        ...
        if include_top:
            for media in self._extract_medias_from_sections((d.get("top", {}) or {}).get("sections", [])):
        ...
        if include_recent and len(posts) < max_posts:
            ...
            while more and next_max_id and page_num < recent_pages and len(posts) < max_posts:
                ...
                sec = self._fetch_hashtag_sections(tag, next_max_id, next_media_ids, page_num)
                if not sec:
                    break
                added = 0
                for media in self._extract_medias_from_sections(sec.get("sections", [])):
                    ...
                    if _add(media, "recent"):
                        out["recent_count"] += 1
                        added += 1
                print(Fore.CYAN + f"   📄 sections page {page_num}: +{added} (total {len(posts)})")
    ...
    def discover(self, query: str, max_hashtags: int = 20, max_users: int = 20) -> Dict:
        ...
        try:
            ...
            result["success"] = True
            print(Fore.GREEN + f"   ✅ {len(result['hashtags'])} hashtag · {len(result['users'])} akun")
        except Exception as e:
            traceback.print_exc()
            result["error"] = str(e)
        return result

    # ── PUBLIC: SEARCH HASHTAG ─────────────────────────────────
    def search_hashtag(self, hashtag: str, max_posts: int = 60,
                       include_top: bool = True, include_recent: bool = True,
                       recent_pages: int = 5) -> Dict:
        ...
        result = {
            ...
            "success": False, "top_count": 0, "recent_count": 0, "total_fetched": 0,
            "posts": [], "related_hashtags": [], "error": None,
        }
        if not tag:
            result["error"] = "Hashtag tidak valid (kosong setelah normalisasi)"
            return result

        print(Fore.CYAN + "\n" + "=" * 70)
        print(Fore.CYAN + f"🔎 SEARCH HASHTAG: #{tag}  (max_posts={max_posts}, recent_pages={recent_pages})")
        print(Fore.CYAN + "=" * 70)
        try:
            self._ensure_ready()
            try:
            ...
            collected = self._collect_hashtag_posts(tag, max_posts, include_top, include_recent, recent_pages)
            if collected.get("error") == "web_info_failed" and not collected["posts"]:
                result["error"] = ("Gagal mengambil data hashtag. "
                                   "Kemungkinan hashtag tidak ada, diblokir, atau sesi perlu di-refresh.")
                return result

            result["media_count"] = collected["media_count"]
            result["formatted_media_count"] = collected["formatted_media_count"]
            result["top_count"] = collected["top_count"]
            result["recent_count"] = collected["recent_count"]
            result["posts"] = collected["posts"]
            result["total_fetched"] = len(collected["posts"])
            result["success"] = True
            print(Fore.GREEN + f"\n✅ #{tag}: {result['total_fetched']} post "
                  f"(top {result['top_count']} · recent {result['recent_count']}) "
                  f"dari total {result['formatted_media_count']}")
        except Exception as e:
            traceback.print_exc()
            result["error"] = str(e)
        return result
    ...
    def search_keyword(self, keyword: str, max_posts: int = 60, max_hashtags: int = 3,
                       per_hashtag_pages: int = 1, include_recent: bool = True) -> Dict:
        result = {
            "query": keyword, "scraped_at": datetime.now().isoformat(),
            "scraped_date": datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
            "success": False, "searched_hashtags": [],
            "suggested_hashtags": [], "suggested_users": [],
            "total_fetched": 0, "posts": [], "error": None,
        }
        kw = (keyword or "").strip()
        if not kw:
        ...
        try:
            ...
            if not candidate_tags:
                ...
                if fb:
                    candidate_tags = [fb]
            if not candidate_tags:
                result["error"] = "Tidak ada hashtag relevan ditemukan untuk keyword ini."
                return result

            chosen = candidate_tags[:max_hashtags]
            print(Fore.CYAN + f"   🏷️  Hashtag dipakai: {', '.join('#' + t for t in chosen)}")

            seen, agg = set(), []
            for idx, tag in enumerate(chosen, 1):
                print(Fore.CYAN + f"\n   [{idx}/{len(chosen)}] mengambil #{tag} ...")
                collected = self._collect_hashtag_posts(
                    tag, max_posts=max_posts, include_top=True,
                    include_recent=include_recent, recent_pages=per_hashtag_pages,
                )
                result["searched_hashtags"].append(tag)
            ...
            # Ranking gabungan: dari like terbanyak (lalu komentar)
            agg.sort(key=lambda x: (x.get("like_count", 0), x.get("comment_count", 0)), reverse=True)
            agg = agg[:max_posts]
            ...
            result["success"] = True
            print(Fore.GREEN + f"\n✅ '{kw}': {len(agg)} post teragregasi dari {len(chosen)} hashtag")
        except Exception as e:
            traceback.print_exc()
            result["error"] = str(e)
        return result
    ...
    def run(self):
        print(Fore.CYAN + "\n" + "=" * 70)
        print(Fore.CYAN + "  INSTAGRAM SEARCH SCRAPER — keyword / hashtag")
        print(Fore.CYAN + "=" * 70)
        while True:
            print(Fore.CYAN + "\n📋 MENU")
            print("  1. Search by Hashtag")
            print("  2. Search by Keyword")
            print("  3. Discover (saran hashtag/akun)")
            print("  4. Exit")
            choice = input("\nPilih [1-4]: ").strip()
            if choice == "1":
                ...
                if not tag:
                    continue
                mp = int(input("Max posts [60]: ").strip() or 60) if input else 60
                rp = 5
                res = self.search_hashtag(tag, max_posts=mp, recent_pages=rp)
                self._save(res, f"search_tag_{self._normalize_hashtag(tag)}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json")
            elif choice == "2":
