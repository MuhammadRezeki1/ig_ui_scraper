...
# IndoBERT + Kamus + Sarcasm Detection + Gemini 2.0 Flash
# ============================================================
# Strategi 4-layer:
...
class SentimentAnalyzerV2:
    ...
    def __init__(self, mode: str = "hybrid", verbose: bool = True):
        ...
        # Typed as Any karena optional runtime imports
        self.ml_model:    Any = None
        self.ml_pipeline: Any = None
    ...
    def _load_indobert(self) -> None:
        try:
            ...
            # Semua import dalam try-block; cast ke Any agar Pylance tidak
            # mencoba meresolve overload dari stubs yang incomplete.
            import importlib
            transformers = importlib.import_module("transformers")
            torch        = importlib.import_module("torch")

            pipeline_fn  = transformers.pipeline                          # type: ignore[attr-defined]
            AutoTokenizer = transformers.AutoTokenizer                    # type: ignore[attr-defined]
            AutoModel     = transformers.AutoModelForSequenceClassification  # type: ignore[attr-defined]
            ...
            self.ml_model  = AutoModel.from_pretrained(model_name)
            ...
            # Panggil pipeline via variabel Any → tidak ada overload resolution
            self.ml_pipeline = pipeline_fn(
                "sentiment-analysis",
                model=self.ml_model,
    ...
    def _load_gemini(self) -> None:
        ...
        try:
            # Import via importlib agar Pylance tidak analisis private exports
            import importlib
            genai = importlib.import_module("google.generativeai")

            # configure & GenerativeModel diakses sebagai attr Any
            genai.configure(api_key=api_key)  # type: ignore[attr-defined]

            # Buat generation_config sebagai dict biasa;
            # google-generativeai menerima plain dict pada runtime
            ...
            self.gemini_model = genai.GenerativeModel(  # type: ignore[attr-defined]
            ...
            if self.verbose:
                print(f"✅ Gemini 2.0 Flash loaded (threshold: confidence < {self.gemini_threshold})")
    ...
    def _has_wellwish_indicator(self, text: str) -> bool:
        if "?" in text:
            return False
        return any(p.search(text) for p in WELLWISH_PATTERNS)
    ...
    def _check_hate_context(self, hate_findings: List[Dict], full_text: str) -> List[str]:
        ...
        for h in hate_findings:
            w        = h["word"]
            safe_lst = SAFE_PHRASES.get(w, [])
            is_safe  = any(safe in text_lower for safe in safe_lst)
    ...
    def analyze_sentiment(self, text: str) -> Dict:
        ...
        if is_hate_speech:
            ...
            if ml_result:
                ...
                sentiment       = self._rule_based_sentiment(
                    positive_words, negative_words, has_sarcasm, has_wellwish
                )
