...
import type { PostResult, Comment, UnifiedResult } from '@/types'
...
type Mode = 'single' | 'batch' | 'unified'
...
// ─────────────────────────────────────────────────────────────────────────────
// NumberInput — slider + direct number input
// ─────────────────────────────────────────────────────────────────────────────
    ...
    <div>
      ...
      <div className="flex items-center gap-3">
        ...
        <input
          ...
          onBlur={handleBlur}
          className={`
            w-20 shrink-0 bg-white/5 border rounded-xl px-3 py-1.5 text-sm text-white text-center
            outline-none transition-colors disabled:opacity-30 disabled:cursor-not-allowed
...
// ─────────────────────────────────────────────────────────────────────────────
// LikersButton — navigasi ke halaman likers
// ─────────────────────────────────────────────────────────────────────────────

function LikersButton({ postUrl, likes, size = 'normal' }: {
  postUrl: string
  likes?: number
  size?: 'normal' | 'small'
}) {
  ...
  return (
    ...
    >
      ...
      Siapa yang Like
      {likes != null && likes > 0 && (
        <span className="text-pink-400/60 text-xs font-normal">
...
// ─────────────────────────────────────────────────────────────────────────────
// DownloadBar — tombol download per hasil
// ─────────────────────────────────────────────────────────────────────────────
...
function DownloadBar({
  ...
  async function handleInlineComments() {
    ...
    setItemState('csv_comments', 'loading')
    try {
      await downloadCommentsInline(comments, filenameHint || 'comments', includeReplies ?? true)
      setItemState('csv_comments', 'done')
    } catch { setItemState('csv_comments', 'error') }
  }

  async function handleInlineLikers() {
    if (!likers?.length) return
    setItemState('csv_likers', 'loading')
    try {
      await downloadLikersInline(likers, filenameHint || 'likers')
      setItemState('csv_likers', 'done')
  ...
  const hasSavedFile = Boolean(savedFile)

  return (
    <div className="flex flex-wrap gap-2 items-center">
      <span className="text-xs text-white/30 uppercase tracking-wider mr-1">Download:</span>

      {/* CSV komentar — inline (dari data di client) */}
      {hasComments && (
        <button
          onClick={handleInlineComments}
          disabled={dlState['csv_comments'] === 'loading'}
      ...
      {hasLikers && (
        <button
          onClick={handleInlineLikers}
          disabled={dlState['csv_likers'] === 'loading'}
      ...
      {/* Server-side downloads jika ada saved file */}
      {hasSavedFile && mode === 'unified' && (
        <>
          <button
            onClick={() => openUrl(downloadUnifiedJson(savedFile!))}
          ...
          {hasComments && (
            <button
              onClick={() => openUrl(downloadUnifiedCommentsCsv(savedFile!, includeReplies ?? true))}
          ...
          {hasLikers && (
            <button
              onClick={() => openUrl(downloadUnifiedLikersCsv(savedFile!))}
              className="btn-glass text-xs flex items-center gap-1.5"
            >
      ...
      {hasSavedFile && mode === 'post' && (
        <>
          <button
            onClick={() => openUrl(downloadPostJson(savedFile!))}
            className="btn-glass text-xs flex items-center gap-1.5"
          >
            <Download size={12} /> JSON Lengkap
          </button>
          {hasComments && (
            <button
              onClick={() => openUrl(downloadPostCommentsCsv(savedFile!, includeReplies ?? true))}
              className="btn-glass text-xs flex items-center gap-1.5"
            >
              <Download size={12} /> CSV Komentar (Server)
            </button>
...
// ─────────────────────────────────────────────────────────────────────────────
// SentimentDetailGrid — detail breakdown sentimen
// ─────────────────────────────────────────────────────────────────────────────
      ...
      {items.slice(0, 5).map((c, i) => (
        <div key={i} className="flex gap-3 items-start py-2 border-b border-white/4 last:border-0">
          ...
          <div className="flex-1 min-w-0">
            ...
            <p className="text-sm text-white/50 break-words">{c.text}</p>
          </div>
...
// ─────────────────────────────────────────────────────────────────────────────
// LikersPreview — preview daftar likers
// ─────────────────────────────────────────────────────────────────────────────

function LikersPreview({ likers, fetched, total }: {
  likers: UnifiedResult['likers']
  fetched: number
  total: number
}) {
    ...
    <div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
        {shown.map((l, i) => (
          ...
          >
            <div className="w-7 h-7 rounded-full bg-gradient-to-br from-pink-500/30 to-purple-500/30 flex items-center justify-center shrink-0 text-xs font-bold text-white/60">
            ...
            <div className="flex-1 min-w-0">
              ...
              {l.full_name && (
                <p className="text-[11px] text-white/30 truncate">{l.full_name}</p>
              )}
            </div>
            {l.is_private && (
...
// ─────────────────────────────────────────────────────────────────────────────
// PostResultView — tampilan hasil single/unified
// ─────────────────────────────────────────────────────────────────────────────
...
function PostResultView({
  ...
  result: PostResult | UnifiedResult
  mode: 'post' | 'unified'
}) {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4">
        ...
        {totalReplies > 0 && (
          <StatCard label="Balasan"   value={totalReplies}          color="blue"   />
        )}
        {likersFetched > 0 && (
          <StatCard label="Likers Diambil" value={likersFetched}   color="pink"   />
        )}
        {result.video_views > 0 && (
          <StatCard label="Video Views" value={result.video_views} color="blue"   />
        )}
        {result.saves_count  > 0 && (
          <StatCard label="Saves"     value={result.saves_count}   color="orange" />
        )}
        {result.shares_count > 0 && (
          <StatCard label="Shares"    value={result.shares_count}  color="yellow" />
        )}
        {result.play_count   > 0 && (
          <StatCard label="Play"      value={result.play_count}    color="blue"   />
        )}
      </div>
      ...
      {s?.top_liked?.length > 0 && (
        <div className="glass-card p-6">
          <h3 className="font-semibold mb-4 text-sm uppercase tracking-widest text-white/50">
            🔥 Top Komentar by Likes
          </h3>
          <TopCommentsList items={s.top_liked} />
        </div>
      )}

      {/* Likers Preview (unified only) */}
      ...
      <div className="glass-card p-6">
        ...
        >
          <span>
            ...
            {totalReplies > 0 && (
              <span className="text-white/40 font-normal ml-1">+ {totalReplies} balasan</span>
            )}
          </span>
...
function BatchResultItem({ item, idx, openIdx, setOpenIdx }: {
  ...
  const replyTotal = d
    ? (d.replies_count ?? (d.comments || []).reduce((acc, c) => acc + (c.replies?.length || 0), 0))
    : 0
  const router = useRouter()
  ...
  return (
    <div className="glass-card p-5 space-y-4">
      ...
      <DownloadBar
        comments={d.comments}
        mode="post"
        savedFile={d._meta?.saved_file}
        filenameHint={d.owner_username || d.shortcode}
        includeReplies={d.include_replies ?? true}
      />
        ...
        <div>
          ...
          >
            <span>
              ...
              {replyTotal > 0 && (
                <span className="text-white/40 font-normal ml-1">+ {replyTotal} balasan</span>
              )}
            </span>
...
// ─────────────────────────────────────────────────────────────────────────────
// LikersConfig — panel konfigurasi likers (untuk unified + standalone)
// ─────────────────────────────────────────────────────────────────────────────

interface LikersConfig {
  scrapelikers: boolean
  maxLikers: number
  aggressiveLikers: boolean
  checkpointSize: number
  checkpointDelayMin: number
  checkpointDelayMax: number
  pageDelayMin: number
  pageDelayMax: number
}

function LikersConfigPanel({
  config,
  onChange,
  disabled,
  forceEnabled,
  ...
  config: LikersConfig
  onChange: (c: LikersConfig) => void
  disabled?: boolean
  forceEnabled?: boolean
}) {
  const set = <K extends keyof LikersConfig>(key: K, value: LikersConfig[K]) =>
  ...
  return (
    <div className="space-y-5">
      {!forceEnabled && (
        <Toggle
          checked={config.scrapelikers}
          onChange={v => set('scrapelikers', v)}
          disabled={disabled}
          label="Scrape Likers"
      ...
      {active && (
        <>
          ...
          <div className="glass rounded-xl p-4 space-y-4">
            <Toggle
              checked={config.aggressiveLikers}
              onChange={v => set('aggressiveLikers', v)}
              disabled={disabled}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
            <NumberInput
              label="Max Likers"
              description="0 = ambil semua (unlimited)"
              value={config.maxLikers}
            ...
            <NumberInput
              ...
              description="Jeda setiap N likers (anti-deteksi)"
            ...
            <NumberInput
              ...
              value={config.checkpointDelayMin}
              min={3} max={60} step={1}
              disabled={disabled}
              accentColor="blue"
            ...
            <NumberInput
              ...
              value={config.checkpointDelayMax}
          ...
          <div className="glass rounded-xl px-4 py-3 flex flex-wrap gap-4 text-xs text-white/40">
            ...
            <span>
              Target: <span className="text-white/70 font-medium">{config.maxLikers === 0 ? 'Semua' : config.maxLikers.toLocaleString('id-ID')}</span>
            </span>
            <span>
              Checkpoint jeda: <span className="text-white/70 font-medium">{config.checkpointDelayMin}–{config.checkpointDelayMax}s</span>
            </span>
          </div>
        </>
      )}
...
const DEFAULT_LIKERS_CONFIG: LikersConfig = {
  scrapelikers:       true,
  maxLikers:          1000,
  aggressiveLikers:   false,
  checkpointSize:     200,
  checkpointDelayMin: 8,
  checkpointDelayMax: 15,
  pageDelayMin:       1.5,
  pageDelayMax:       3.0,
}
...
export default function ScrapePage() {
  ...
  const handleScrape = useCallback(async () => {
    ...
    try {
      ...
      if (mode === 'single') {
        const resp = await scrapePost(singleUrl, maxComments, opts)
        ...
        const resp = await scrapeUnified({
          ...
          include_replies:          includeReplies,
          max_replies_per_comment:  maxReplies,
          scrape_likers:            likersConfig.scrapelikers,
          max_likers:               likersConfig.maxLikers,
          aggressive_likers:        likersConfig.aggressiveLikers,
          checkpoint_size:          likersConfig.checkpointSize,
          checkpoint_delay_min:     likersConfig.checkpointDelayMin,
          checkpoint_delay_max:     likersConfig.checkpointDelayMax,
          page_delay_min:           likersConfig.pageDelayMin,
          page_delay_max:           likersConfig.pageDelayMax,
        })
        if (!resp.success) throw new Error(resp.message)
        setResult(resp.data)
        ...
        const data = resp.data as {
          total?: number; success?: number; failed?: number; results?: BatchItem[]
        }
  ...
  }, [mode, url, batchUrls, maxComments, includeReplies, maxReplies, likersConfig])
  ...
  const tabs: { key: Mode; label: string; icon: React.ReactNode; desc: string }[] = [
    {
      ...
      desc:  'Komentar + balasan satu post',
    },
    {
      key:   'unified',
      label: 'Unified Scrape',
      icon:  <Layers size={14} />,
      desc:  'Komentar + likers sekaligus',
    },
    {
      key:   'batch',
      label: 'Batch',
      icon:  <BarChart2 size={14} />,
      desc:  'Multiple URL sekaligus',
    },
  ]
  ...
  return (
    <div className="p-8 max-w-5xl">
      ...
      <div className="flex items-center gap-3 mb-8">
        ...
        <div>
          ...
          <p className="text-sm text-white/40">
            Komentar · Balasan · Sentimen · Likers
          </p>
        </div>
      </div>
      ...
      {/* Mode tabs */}
      <div className="glass-card p-1.5 inline-flex mb-6 gap-1">
        {tabs.map(tab => (
          <button
            key={tab.key}
          ...
          >
            ...
            {tab.label}
            {tab.key === 'unified' && (
              <span className="text-[10px] bg-pink-500/20 text-pink-300 px-1.5 py-0.5 rounded-full font-normal">
                NEW
              </span>
            )}
          </button>
      ...
      <div className="glass-card p-6 mb-6 space-y-6">
              ...
              {batchUrls.map((u, i) => (
                <div key={i} className="flex gap-2">
                  <div className="relative flex-1">
                    ...
                    <input
                      type="url" value={u} disabled={disabled}
                      onChange={e => {
                        const copy = [...batchUrls]; copy[i] = e.target.value; setBatchUrls(copy)
                      }}
                      placeholder={`URL #${i + 1}`}
        ...
        <div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-5">
            <NumberInput
              label="Max Komentar"
              description="Jumlah komentar utama yang diambil"
              value={maxComments} min={10} max={500} step={10}
              disabled={disabled} accentColor="pink"
            ...
            <NumberInput
              ...
              disabled={disabled || !includeReplies} accentColor="purple"
              onChange={setMaxReplies}
            />
          </div>

          {/* Summary + toggle balasan */}
          <div className="flex flex-wrap items-center gap-4">
            <div className="glass rounded-xl px-4 py-2.5 flex flex-wrap gap-4 text-xs text-white/40 flex-1">
              <span className="flex items-center gap-1.5">
                ...
                Komentar: <span className="text-white/70 font-medium ml-1">{maxComments}</span>
              </span>
              {includeReplies && (
                <span className="flex items-center gap-1.5">
            ...
            <Toggle
              ...
              icon={<MessageCircle size={13} className="text-white/40" />}
            />
          </div>
        </div>
      ...
      {loading && (
        <div className="glass-card p-12 text-center mb-6">
          ...
          <p className="text-white/30 text-xs mt-1">
            {mode === 'unified' && likersConfig.scrapelikers
              ? `Komentar + likers (est. target ${likersConfig.maxLikers === 0 ? 'semua' : likersConfig.maxLikers.toLocaleString('id-ID')} likers) — bisa 5–15 menit`
              : includeReplies
              ? 'Komentar + balasan, bisa 1–3 menit'
              : mode === 'batch'
              ? 'Batch bisa makan beberapa menit'
              : 'Sekitar 30–60 detik'}
          </p>
          <p className="text-white/20 text-xs mt-1">Jangan pindah halaman agar hasil langsung tampil.</p>
      ...
      {result && (
        <PostResultView result={result} mode={resultMode} />
      )}
        ...
        <div className="space-y-5">
          ...
          {batchResults.map((item, idx) => (
            <BatchResultItem
              ...
              item={item}
              idx={idx}
              openIdx={openComments}
              setOpenIdx={setOpenComments}
            />
          ))}
        </div>
      )}
